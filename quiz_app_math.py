...
csv_file_path = r"E:\quiz_app_python\quiz_math\real numbers 100 mcqs CSV.csv"
...
def load_csv_file(file_path):
    ...
    if missing_columns:
        print(f"Warning: Missing columns {missing_columns}. Available columns: {df.columns.tolist()}")
    ...
    if 'Correct Answer' in df.columns:
        ...
        print("Sample Correct Answer values after preprocessing (first character only):", df['Correct Answer'].head().tolist())
    else:
        print("Warning: 'Correct Answer' column not found in CSV. Available columns:", df.columns.tolist())
    
    print(f"Total questions: {len(df)}")
    return df

# Load questions from the CSV file
all_questions = load_csv_file(csv_file_path)
