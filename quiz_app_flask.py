...
directory_path = r"E:\quiz_app_python\quiz_math\real numbers 100 mcqs CSV.csv"
...
def load_all_csv_files(directory_path):
    ...
    if 'Correct Answer' in combined_df.columns:
        ...
        combined_df['Correct Answer'] = combined_df['Correct Answer'].astype(str).str.strip().str.replace(r'[\r\n\t\s]+', '', regex=True).str[0].str.upper()
        # Log the processed values for debugging
        print("Sample Correct Answer values after preprocessing (first character only):", combined_df['Correct Answer'].head().tolist())
    else:
        print("Warning: 'Correct Answer' column not found in CSV. Available columns:", combined_df.columns.tolist())
